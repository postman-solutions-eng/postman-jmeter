...
function updateCollection(responseCollection, opts)
{
  ...
  let body = "";
  let collName = opts.collectionPath.split('/')[opts.collectionPath.split('/').length - 1]
  ...
  findAllRequests(responseCollection, "request");
   ...
   let newCollection = {
    "collection": {
        ...
        "item": [
            {
                "name": "SCM Workflow Run (" +opts.workflowName + ": " + opts.workflowRunNumber + ") - " + collName,
                "event": [
                  {
                    "listen": "test",
                    "script": {
                      "exec": [
                        ...
                        "    <img src=\"" + opts.resultsURL + "?raw=true\" height=\"95%\" width=\"95%\">`",
