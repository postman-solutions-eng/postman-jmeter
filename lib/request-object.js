...
RequestObject.prototype.parseBody = function () {

  let params = [];

  if(typeof this.pmRequestItem.request.body === 'undefined')
  {
    return;
  }

  params = this.pmRequestItem.request.body.raw.match(this.pmVarRegEx);
  let curParam = "";
  if(params === null)
  {
    return;
  ...
  params.forEach(param => {
    curParam = this.pmVariables.get(this.stripPmVarKey(param));
    if(this.varMode = 'transform')
    {
      this.pmRequestItem.request.body.raw = this.pmRequestItem.request.body.raw.replaceAll(param, curParam.getJmeterVariableToken());
    }
    else if(this.varMode = 'resolve')
    {
      this.pmRequestItem.request.body.raw = this.pmRequestItem.request.body.raw.replace(param, curParam.getValue());
    }
...
RequestObject.prototype.parseParameters = function () {
  if(typeof this.pmRequestItem.request.url.query === 'undefined') // no params
  {
    return;
  }
...
RequestObject.prototype.getBodyData = function () {
  ...
  retval.push({"bodyText":this.pmRequestItem.request.body.raw})
  return retval;
}

RequestObject.prototype.transformToJmeterRequest = function () {
  let serverName = this.varMode === 'resolve' ? this.url.host : this.jmServerString
  serverName = serverName.replace(this.httpProtocolRegEx, "");
  const view = {
    testName: this.name,
    server: serverName,
    ...
    paramHeader: this.requestParams.length === 0 ? false : true
